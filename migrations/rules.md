...
- Storybook 10.6 (`config/storybook/main.ts`, `preview.ts`, `webpack.config.ts`), 30 stories. Визуальные тесты Loki/`reg-cli` удалены на этапе 7.
...
| 6a. ESLint 10 flat config | [x] | `feature/YH-2415` (не закоммичено) | eslint 10.10 + `eslint.config.mjs` (flat). `typescript-eslint` 8.70, `eslint-plugin-react-hooks` 7.1.1, `eslint-plugin-react` 7.37.5, `eslint-plugin-jsx-a11y` 6.10.2, `eslint-config-prettier` 10.1.8, `globals` 17.12. **Отклонение (1):** `@feature-sliced/eslint-config` и `@conarti/eslint-plugin-feature-sliced` удалены — eslintrc-only, вызывают снятые API ESLint 10 (`ecmaVersion: "2015"` строкой, `context.getFilename`). Слои/public-api покрывает steiger; `import/no-internal-modules` и experimental `import/order` перенесены в flat-конфиг. **Отклонение (2):** `eslint-plugin-import` 2.32 не поддерживает ESLint 10 (`getTokenOrCommentAfter`) — заменён на `eslint-plugin-import-x` 4.17, зарегистрирован как `import` (disable-комментарии валидны). **Отклонение (3):** `overrides` для peer `eslint` у `eslint-plugin-react` / `eslint-plugin-jsx-a11y` (peer до ^9, код совместим). **Отклонение (4):** `settings.react.version: '19.3'` вместо `'detect'` — `detect` зовёт `context.getFilename()`. Правила React Compiler из recommended оставлены, шумные (`set-state-in-effect`, `refs`, `purity`, `static-components`, `immutability`, …) временно `'warn'` с TODO. `rules-of-hooks` error, `exhaustive-deps` warn. Тривиальные ошибки v8 recommended исправлены (unused `import React`, short-circuit как statements, `no-empty-object-type`, мёртвый `??` после `Number()`). Итог: 0 ошибок / **130 warnings** (было 173: −import/order, +compiler). `npx eslint --print-config src/app/index.tsx` — плагины js, typescript-eslint, react, react-hooks, jsx-a11y, prettier, import-x. |
| 6b. Stylelint 17 | [x] | `feature/YH-2415` (не закоммичено) | stylelint 17.15, `stylelint-config-standard` 40, `stylelint-order` 8.1.1, `stylelint-declaration-strict-value` 1.12.1 (пин с этапа 1 снят). Правило `indentation` удалено. 6 новых ошибок standard 40 исправлены руками: `word-break: break-word` → `overflow-wrap`, `@media (width <= 360)` → `360px`, `min-width: none` → `unset`, quoted `transition`, порядок селекторов `no-descending-specificity`. `lint:css` — 0 ошибок. |
| 6c. Прочий тулинг | [x] | `feature/YH-2415` (не закоммичено) | lint-staged **16.4.0** (не 17: engines Node ≥22.22.1, локально 22.17.1). `@commitlint/*` 21.2.2; `commitlint.config.ts` → `.cjs` (без ts-node). `postcss.config.ts` → `.cjs`. `.lintstagedrc` → `.lintstagedrc.json`. steiger 0.6 + `@feature-sliced/steiger-plugin` 0.7: новое `fsd/segments-by-purpose` (providers/assets) временно `'warn'` с TODO. steiger: 0 ошибок / **24 warnings** (было 20 + 4 новых). loki 0.35.1. |
| 6d. TypeScript 6 | [x] | `feature/YH-2415` (не закоммичено) | typescript **6.0.3** (не 7). `target: ES2020`, `lib: [ES2022, DOM, DOM.Iterable]`, `moduleResolution: bundler`. `baseUrl` удалён (TS5101 deprecated в 6.0). Маппинг `@tiptap/react/menus` с этапа 5e снят — bundler читает `exports`. `skipLibCheck: true` оставлен (monaco/storybook/goober/type-fest). **Отклонение (1):** `ts-node` не удалялся — webpack.config.ts на ESM-синтаксисе, без `"type": "module"` нативный type-stripping Node трактует `.ts` как CJS; локальный Node 22.17.1 < 22.18. Секция `ts-node.compilerOptions` — `module: CommonJS` + `moduleResolution: node`. **Отклонение (2):** `declare module '@testing-library/user-event'` в `global.d.ts` — транзитивный 14.3 без `exports.types`. **Отклонение (3):** Storybook 7 `typescript.reactDocgen: 'react-docgen'` — `react-docgen-typescript` + TS 6 падает (`Paths must either both be absolute or both be relative`); вернуть на этапе 7. Чанки ≈ этапа 5e (`css/main` 114692, −23 байта из правок CSS). `npm run dev:http` → `compiled successfully`. Ручной smoke — за владельцем. |
| 7. Storybook 10 | [x] | `feature/YH-2415` (не закоммичено) | storybook / `@storybook/react-webpack5` / `@storybook/addon-docs` / `@storybook/addon-links` / `eslint-plugin-storybook` **10.6.0**, `@storybook/addon-webpack5-compiler-swc` 4.0.3, `@storybook/addon-styling-webpack` 3.0.2, `storybook-react-i18next` 10.1.4. Путь 7→8.6.18→9.1.20→10.6.0 через `npx storybook@N upgrade`. **Главное отклонение: Loki и все скриншотные тесты удалены**, не заменялись storycap/Playwright/Chromatic: пакеты `loki`/`reg-cli`, секция `loki` в `package.json`, скрипты `test:ui*`, `scripts/generate-visual-json-report.js.js`, каталог `.loki/` (~164 эталона), закомментированный шаг Visual tests в `pr-checks.yml`. Автомиграция 8 добавила `@chromatic-com/storybook` — убран. Автомиграция 10 добавила `@storybook/addon-mcp` — убран. Удалены `@storybook/addon-essentials`/`interactions`/`onboarding`/`blocks`/`test`/`icons` (icons не использовался в stories). Сняты `overrides` этапа 4 (react peer SB7) и этапа 5b (`storybook-react-i18next` vs i18next 26) — SB 10 и i18next-addon 10.1.4 поддерживают React 19 / i18next 26 / react-i18next 17. Конфиг ESM: `import.meta.dirname`, `import webpack from 'webpack'` (named ESM-export `DefinePlugin` нет), относительный импорт `./webpack.config.ts`. CSS Modules в `addon-styling-webpack`: `namedExport: false`, `exportLocalsConvention: 'as-is'`, postcss-loader. `useSWC` убран, компилятор только через SWC-аддон + `runtime: 'automatic'`. `docs.autodocs` → теги в stories; `globals` → `initialGlobals` (locale + backgrounds). Импорты историй `@storybook/react` → `@storybook/react-webpack5` (automigration renderer-to-framework). `argTypesRegex` удалён; обязательные колбэки в stories — `fn()` из `storybook/test` (Radio/Range/Switch/Stub). `reactDocgen: 'react-docgen'` оставлен (этап 6d). `eslint-plugin-storybook` 10.6.0 подключён как `flat/recommended`. `npm ls --depth=0` чистый, `npm ci` без флагов (только известный override eslint-plugin-react). `tsc` 0, ESLint 0 / **130 warnings** (=), Stylelint 0, steiger 0 / **24 warnings** (=), prettier чистый, 56/56 наборов, 555/555 тестов. `storybook:build` зелёный (30 stories + autodocs). `npm run build` — чанки байт-в-байт как этап 6d: `runtime` 8021, `react` 218801, `text-editor` 599848, `core` 609509, `ui` 83437, `vendors` 217398, `main` 793486, `css/main` 114692. 2 warning'а `act` — отклонение этапа 4. `npm run dev:http` → `compiled successfully`. Ручной smoke Storybook (все 30 историй, locale/background, controls/docs) — за владельцем. |
...
| storybook и @storybook/* | 10.6.0 | 10.6.0 | 7 |
| storybook-react-i18next | 10.1.4 | 10.1.4 | 7 |
| loki | удалён на этапе 7 | — | 7 |
