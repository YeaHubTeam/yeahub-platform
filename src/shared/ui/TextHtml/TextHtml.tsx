...
export const TextHtml = ({
	...
	useEffect(() => {
		...
		const processedHtml = isMarkdown ? markdownToHtml(html) : normalizeHtmlContent(html);
					...
					copyButton.onclick = () => {
						if (!copyButton.disabled) {
							navigator.clipboard.writeText(codeContent).then(() => {
								...
								setTimeout(() => copyButton.classList.remove(styles['copied']), 2000);
							});
						}
					};
	...
	}, [html, disableCodeCopy, isMarkdown]);
