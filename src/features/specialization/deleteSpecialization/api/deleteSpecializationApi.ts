...
import { getDeleteSpecializationApiErrorMessage } from '../lib/utils/getDeleteSpecializationApiErrorMessage';
		...
		deleteSpecialization: build.mutation<void, number>({
			...
			async onQueryStarted(_, { queryFulfilled, extra }) {
				try {
					...
					toast.error(i18n.t(handleApiError(error, getDeleteSpecializationApiErrorMessage)));
