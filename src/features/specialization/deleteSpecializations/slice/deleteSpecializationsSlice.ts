...
export const deleteMultipleSpecializations = createAsyncThunk<void, SelectedAdminEntities>(
			...
			const responses = await Promise.allSettled(
				specializations.map(
					async (specialization) =>
						await dispatch(
							deleteSpecializationsApi.endpoints.deleteSpecializationWithoutErrorHandler.initiate(
								specialization.id,
							),
						),
				),
