...
import { ClientType, clientTypes, CreateOrEditFeatureFlagFormValues } from '@/entities/featureFlag';

export const featureFlagEditSchema: yup.ObjectSchema<CreateOrEditFeatureFlagFormValues> = yup
	.object()
	.shape({
		id: yup.string().required(i18n.t(Translation.VALIDATION_REQUIRED)),
		flag: yup
			.string()
			.trim()
