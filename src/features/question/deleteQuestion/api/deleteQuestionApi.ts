...
import { getDeleteQuestionApiErrorMessage } from '../lib/utils/getDeleteQuestionApiErrorMessage';
		...
		deleteQuestion: build.mutation<void, Question['id']>({
			...
			async onQueryStarted(_, { queryFulfilled, extra }) {
				try {
					...
					toast.error(i18n.t(handleApiError(error, getDeleteQuestionApiErrorMessage)));
