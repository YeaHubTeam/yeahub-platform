...
export const TopicsTable = ({ topics, selectedTopics, onSelectTopics }: TopicsTableProps) => {
	...
	const renderTableBody = (topic: Topic) => {
		...
		return Object.entries(columns).map(([k, v]) => (
			<td key={k} className={styles.cell}>
				{k === 'title' ? (
					<Link to={route(ROUTES.admin.topics.details.route, topic.id)}>
	...
	const renderActions = (topic: UITopic) => {
		const menuItems: PopoverMenuItem[] = [
			{
				icon: <Icon icon="eye" size={24} />,
				title: t(Translation.SHOW, { ns: i18Namespace.translation }),
				onClick: () => {
					navigate(route(ROUTES.admin.topics.details.route, topic.id));
				},
			},
			{
				...
				onClick: () => {
					navigate(route(ROUTES.admin.topics.edit.route, topic.id));
				},
			},
			// {
